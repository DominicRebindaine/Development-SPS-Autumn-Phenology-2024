...
resultsLM.LOD = BudsSummary %>% 
  do({model = lme4::lmer(EOS.DOY ~ lo.doy + (1|bud.type) + (1|treat.cool.combined), data=.)  # create the model
  data.frame(tidy(model, effect="fixed"))}) %>%
  filter(!term %in% c("(Intercept)")) %>% 
  mutate(SEx2 = std.error*2)
...
resultsLM.LOD.Control = BudsSummary %>% filter(group %in% c('L','M')) %>% 
  do({model = lme4::lmer(EOS.DOY ~ lo.doy + (1|bud.type), data=.)  # create the model
  data.frame(tidy(model, effect="fixed"))}) %>%
  filter(!term %in% c("(Intercept)")) %>% 
  mutate(SEx2 = std.error*2)
...
LMplotBudSetModerate = ggplot() + 
  ...
  theme(panel.spacing.x = unit(0,"cm"), axis.text.x = element_blank(), axis.ticks.x = element_blank(), 
        ...
        legend.position = c(.82, .18), legend.title = element_text(size = 20), legend.text = element_text(size = 20)
       )
...
LMplot90.LvM = ggplot() + 
  ...
  xlab("")+ylab("Bud set advancement (days)")+
  plotTheme1 +
   theme(legend.position = 0, panel.border = element_blank(), axis.text = element_text(size = 18), axis.title.y = element_text(size = 20))
...
LMplot90.NvO = ggplot() + 
 scale_color_manual(values = 'dodgerblue3')+
  ...
  xlab("")+ylab("")+
  plotTheme1 +
   theme(legend.position = 0, axis.line.y = element_blank(), axis.text.y = element_blank(), panel.border = element_blank(), axis.ticks.y = element_blank(), axis.text = element_text(size = 18))
...
LMplot90.PvQ = ggplot() + 
 scale_color_manual(values = 'dodgerblue3')+
  ...
  xlab("")+ylab("")+
  plotTheme1 +
   theme(legend.position = 0) +theme(legend.position = 0, axis.line.y = element_blank(), axis.text.y = element_blank(), panel.border = element_blank(), axis.ticks.y = element_blank(), axis.text = element_text(size = 18))
...
BudsSummary %>% ggplot(aes(lo.timing, EOS, fill = lo.timing)) + 
  ...
  plotTheme1 + theme(legend.position = 0, axis.text = element_text(size = 20), axis.title = element_text(size = 22), axis.text.y = element_text(angle = 15))
...
# Merge separated bud set anomaly plots with each treatment compared to its respective control as well as early vs late boxplot
 
 layout = "AAA
 BBB"
 
 CombinedBox_AnomalyPlot = lo.eos.boxplot + LMplotBudSetModerate +
